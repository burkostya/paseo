...
| Retained plan cards      | central stream projection for app-only `permission_plan` items                     | No wire change                                                                                 | Reconstructed from the loaded permission-event timeline                                               | renders resolved and rejected plan permission events as durable timeline items                                                                    |
| Sidebar action parity    | `SidebarRowAction[]` builders and dropdown/context renderers                       | No wire change                                                                                 | None                                                                                                  | exposes one action model for kebab and context menus, including pin/unpin and all project/workspace actions                                       |
...
| Prompt navigator         | `StreamViewportHandle.scrollToItem(itemId, align)`                                 | No wire change                                                                                 | None                                                                                                  | provides a virtualizer-neutral prompt marker navigator for the web/desktop stream                                                                 |

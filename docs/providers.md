...
Pi's `agent_end` event closes one low-level agent run, not necessarily the user-visible turn: automatic retry, overflow compaction, or queued continuation may still follow. The adapter keeps the Paseo turn active through those events and settles it on `agent_settled`. A temporary compatibility fallback handles Pi versions older than 0.80.4, which do not emit `agent_settled`.

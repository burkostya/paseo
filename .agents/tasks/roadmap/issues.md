# Fork extensions (`v0.1.109` baseline)

Detailed contracts, persistence, and removal criteria live in
[`docs/fork-extensions.md`](../../../docs/fork-extensions.md).

- Favorite model synchronization
- Provider usage warnings
- Configurable issue links
- Selectable diff base
- Native Codex commands
- Retained plan cards
- Shared sidebar row actions
- Composer prompt history
- Ctrl-Tab workspace MRU
- Prompt marker navigator
- Desktop/Sway attention

## Open issues

- Ctrl+Shift+T does not reopen closed tabs containing agent sessions
- Messages have no timestamps, making it difficult to inspect conversation timing when needed

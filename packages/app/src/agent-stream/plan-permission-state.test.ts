...
describe("projectPlanPermissionItems", () => {
  it("keeps one skipped request at its original position when it is replayed in the live head", () => {
    ...
    duplicate.turnId = "turn-1";
    const timelineResult = planTimelineResult("plan-1", "Same proposal", "skipped");
    timelineResult.timelineCursor = { epoch: "epoch-1", seq: 42 };
    const newPlan = plan("plan-2", undefined, "Same proposal");

    const result = projectPlanPermissionItems({
      tail: [plan("plan-1", undefined, "Same proposal"), userMessage("u1")],
      head: [duplicate, timelineResult, newPlan],
    });

    expect(result.tail.map((item) => item.kind)).toEqual(["permission_plan", "user_message"]);
    expect(result.head).toEqual([newPlan]);

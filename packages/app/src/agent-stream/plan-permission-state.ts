...
function mergeDuplicatePlanPermissionItem(
  anchor: PermissionPlanItem,
  duplicate: PermissionPlanItem,
): PermissionPlanItem {
  const timelineCursor = duplicate.timelineCursor ?? anchor.timelineCursor;
  const turnId = duplicate.turnId ?? anchor.turnId;
  const resolution = duplicate.resolution ?? anchor.resolution;

  if (
    timelineCursor === anchor.timelineCursor &&
...
function projectPlanPermissionLane(
  ...
  seenPlanPermissionItems: Set<string>,
  ...
  for (const item of items) {
    if (isPermissionPlanItem(item)) {
      const key = planPermissionKey(item.request.provider, item.request.id);
      if (seenPlanPermissionItems.has(key)) {
        changed = true;
        continue;
      }

      seenPlanPermissionItems.add(key);
      const canonical = planItemsByKey.get(key) ?? item;
      const projected = resolvedPlanItemsByKey.get(key) ?? canonical;
      next.push(projected);
...
export function projectPlanPermissionItems(input: {
  ...
  for (const item of allItems) {
    if (isPermissionPlanItem(item)) {
      const key = planPermissionKey(item.request.provider, item.request.id);
      const previous = planItemsByKey.get(key);
      planItemsByKey.set(key, previous ? mergeDuplicatePlanPermissionItem(previous, item) : item);
      continue;
    }
  ...
  const seenPlanPermissionItems = new Set<string>();

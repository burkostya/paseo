...
function buildCodexPlanImplementationPrompt(planText: string): string {
  ...
  const prompt = normalizedPlan
    ...
    : `${CODEX_PLAN_IMPLEMENTATION_PROMPT_PREFIX} Make the required code changes and verify them.`;

  // The implementation turn is provider context generated by approving the plan,
  // not a second user-authored prompt. The manager already filters this envelope
  // from live and replayed timelines while Codex still receives the full content.
  return formatSystemNotificationPrompt(prompt);
}
    ...
    if (resolution.behavior === "deny") {
      ...
      if (typeof planText === "string") {
        this.emitEvent({
          type: "timeline",
          provider: CODEX_PROVIDER,
          ...(pending?.plan?.turnId ? { turnId: pending.plan.turnId } : {}),
          item: {
            type: "tool_call",
            callId: requestId,
            name: "plan_approval",
      ...
      this.recordPlanOutcome(requestId, "approved");
    }
    this.pendingPermissionHandlers.delete(requestId);
    this.pendingPermissions.delete(requestId);
    this.resolvedPermissionRequests.add(requestId);
